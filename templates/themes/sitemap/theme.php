<?php
	...
	function sitemap_build($action, $settings, $board) {
		...
		foreach ($boards as $board) {
			$query = query(sprintf("SELECT `id_for_board` AS `thread_id`, (SELECT `time` FROM ``posts`` WHERE `board` = '%s' AND`thread` = `thread_id` OR `id_for_board` = `thread_id` ORDER BY `time` DESC LIMIT 1) AS `lastmod` FROM ``posts`` WHERE `board` = '%s' AND`thread` IS NULL", $board, $board)) or error(db_error());
